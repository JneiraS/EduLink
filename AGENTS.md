...
- AI assistant (Ollama): `TextAssistantPort` (`app/domain/ports/services.py`, `summarize(texts) -> str`) implemented by `OllamaTextAssistant` (`app/infrastructure/ai/ollama_adapter.py`, local Ollama server — student/parent data never leaves the school network). URL/model come from config (`OLLAMA_BASE_URL`, `OLLAMA_MODEL` env vars); French prompts live in the adapter; network/HTTP errors map to `ServiceUnavailableError` (`app/domain/errors.py`). `POST /ai/channels/<id>/summary` (`ai_routes.py`) → `SummarizeChannelMessages` (membership guard, empty channel → `ValidationError`), rate-limited 10/min/IP, returns JSON errors (`{"error": ...}` + 400/403/404/503) instead of flash+redirect because it is consumed by `fetch` — keep that pattern for future JSON endpoints. UI hook: "Resumer la conversation" button in `channel_detail.html` → `initAiSummary()` in `static/js/app.js` (`data-ai-summary*` attributes, CSRF via the `csrf-token` meta tag, no inline scripts). Tests inject a fake port/use case — never call real Ollama.
