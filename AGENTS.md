...
- Message search: `content_search` column stores plaintext (lowercased, trimmed) for `ILIKE` search — `search_by_channel` queries `content_search`, not `content`. The `EncryptedString` TypeDecorator (when added) will handle encryption/decryption transparently via `process_bind_param`/`process_result_value`.
- Teacher parent lookup: `GET /messages/find-parent` (`messages_routes.py`) → `FindParentsByChild` (garde TEACHER/ADMIN) → `children_repository.find_by_name_like` + `users.find_many_by_ids` → returns `list[ParentSearchResult]` (child + UserSummary). POST opens a direct conversation via `OpenDirectConversation`. Nav link "Contacts" visible only to TEACHER.
- AI assistant (Ollama): `TextAssistantPort` (`app/domain/ports/services.py`, `summarize(texts) -> str`, `rephrase(text) -> str`) implemented by `OllamaTextAssistant` (`app/infrastructure/ai/ollama_adapter.py`, local Ollama server — student/parent data never leaves the school network; shared private `_generate(prompt)` helper). URL/model come from config (`OLLAMA_BASE_URL`, `OLLAMA_MODEL` env vars); French prompts live in the adapter; network/HTTP errors map to `ServiceUnavailableError` (`app/domain/errors.py`). `POST /ai/channels/<id>/summary` → `SummarizeChannelMessages` (membership guard, empty channel → `ValidationError`), `POST /ai/rephrase` → `RephraseDraft` (empty/too-long draft → `ValidationError`; no channel guard — operates on the composer draft, not a stored message). Both rate-limited 10/min/IP and return JSON errors (`{"error": ...}` + 400/403/404/503) instead of flash+redirect because they are consumed by `fetch` — keep that pattern for future JSON endpoints; routes forward payloads verbatim, normalization (strip, length caps) belongs to the use case. UI hooks in `channel_detail.html`: "Resumer la conversation" button (`initAiSummary()`) and composer magic-wand button (`initAiRephrase()` — suggestion panel with "Utiliser cette version" filling the textarea + synthetic `input` event for auto-grow) in `static/js/app.js` (`data-ai-*` attributes, CSRF via the `csrf-token` meta tag, no inline scripts). Tests inject a fake port/use case — never call real Ollama.
